...
head(mf_pps)
```
The actual results file returns a table of each candidate for each polling place, and the `get_mediafeed_preload_pps_cand()` function returns a table in that format, which can be easily merged with the results when they are retrieved. 
...
results_xml <- fetch_mediafeed_results(2020)
```
